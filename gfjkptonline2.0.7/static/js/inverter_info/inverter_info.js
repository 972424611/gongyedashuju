...
/*日期选择模块开始*/
showFieldSelect(dateType);
console.log(nowDate,'day',inverter_name,field);
...
function showChart(date,dateType,name,field) {
    $.ajax({
        ...
        success: function (jsonResult) {
            ...
            var chartTitle = '';
            if(dateType === 'day'){
                titleDateLeft = getNowChineseDate(addDate(date,-1));
                titleDateRight = getNowChineseDate(date);
                chartTitle = titleDateLeft + "至" + titleDateRight + "-" + field + '监测';
...
function showFieldSelect(dateType) {
    ...
    var optionValueArr = [];
    if(dateType === 'day'){
        optionValueArr = ['总有功功率','总发电量','日发电量'];
